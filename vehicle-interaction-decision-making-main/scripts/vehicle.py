...
class Vehicle(VehicleBase):
    ...
    def __init__(self, name, cfg: dict = {}) -> None:
        super().__init__(name)
        self.vehicle_box2d: np.ndarray = VehicleBase.get_box2d(self.state)  # 获取车辆四角坐标
        self.safezone: np.ndarray = VehicleBase.get_safezone(self.state)    # 获取车辆安全距离四角坐标
        self.target: utils.State = utils.State(0, 0, 0, 0)                  # 初始化车辆状态，位置、角度、速度均为0
        self.have_got_target: bool = False                                  # 是否到达目标地点
        self.dt: float = cfg['delta_t']                                     # 车辆移动的时间间隔
        self.cur_action: Optional[utils.Action] = None                      # 当前动作
        self.excepted_traj: Optional[utils.StateList] = None                # 状态列表，期待轨迹
        self.footprint: List[utils.State] = []                              # 车辆的足迹
...
class VehicleList:
    ...
    # 从车辆列表 vehicle_list 中排除指定的车辆，并返回排除后的列表
